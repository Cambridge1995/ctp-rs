...
## 本项目
- **注意**: 本项目原开发者为 [@rn7s2](https://github.com/rn7s2) 的 [ctp-rs](https://github.com/rn7s2/ctp-rs) 项目，本项目只进行了函数、结构体、常量、结构体字段的命名上的改造。
...
- 新增改动:
  1. 将channel从默认std标准库切换为kanal,提升传输效率(事实上可能提升有限,耗时最大的位置在网络传输,二者不在一个量级,提升有限)
  2. 修改wrapper层DepthMarketData结构体字段复制逻辑,行情数据没有中文,在Rust场景下,GBK与UTF-8可以无损转换,不用深拷贝,减少拷贝开销(事实上可能提升有限,只有六个字段)
  3. 字段名改动逻辑:
     1. 结构体去除Field后缀
     2. 常量改为enum枚举,去除掉共同前缀
     3. 结构体字段改为snake_case命名法
     4. 方法名改为snake_case命名法
     5. 修改了各处命名单词拼写错误(IDE有下划线提醒,看着好恼火,既然都桥接了,自然改成正确拼写,我强迫症)
